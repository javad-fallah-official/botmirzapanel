 ...
 * 
 * Represents a unique identifier for a Subscription entity.
 */
class SubscriptionId
{
    ...
    private function validate(string $id): void
    {
        ...
        if (strlen($id) > 255) {
            throw new \InvalidArgumentException('Subscription ID is too long (max 255 characters).');
        }

        // Allow UUIDs, numeric IDs, or alphanumeric strings
        if (!preg_match('/^[a-zA-Z0-9\-_]+$/', $id)) {
            throw new \InvalidArgumentException('Subscription ID contains invalid characters.');
        }
    }
