 ...
 * 
 * Represents a unique identifier for a Payment entity.
 */
class PaymentId
{
    ...
    private function validate(string $id): void
    {
        ...
        if (strlen($id) > 255) {
            throw new \InvalidArgumentException('Payment ID is too long (max 255 characters).');
        }

        // Allow UUIDs, numeric IDs, or alphanumeric strings
        if (!preg_match('/^[a-zA-Z0-9\-_]+$/', $id)) {
            throw new \InvalidArgumentException('Payment ID contains invalid characters.');
        }
    }
